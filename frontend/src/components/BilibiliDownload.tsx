...
const BilibiliDownload: React.FC<BilibiliDownloadProps> = ({ onDownloadSuccess }) => {
  ...
  const parseVideoInfo = async () => {
    if (!url.trim()) {
      setError('Please enter正确的视频链接')
      return
    }

    const videoType = getVideoType(url.trim())
    if (!videoType) {
      setError('Please enter正确的B站或YouTube视频链接')
      return
    }

    setParsing(true)
    setError('') // 清除之前的Error信息
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (task.status === 'completed') {
          ...
          message.success('Video DownloadComplete！')
          ...
          setPollingInterval(null)
          setDownloading(false)
          message.error(`DownloadFailed: ${task.error_message || '未知Error'}`)
          resetForm()
        }
      } catch (error) {
        console.error('轮询任务StatusFailed:', error)
      }
    }, 2000)
  ...
  const handleDownload = async () => {
    if (!url.trim()) {
      message.error('请Enter video URL')
      return
    }

    const videoType = getVideoType(url.trim())
    if (!videoType) {
      message.error('Please enter有效的B站或YouTube视频链接')
      return
    }
    ...
    try {
      ...
      // 检查响应是否包含ProjectID（新的优化后的响应格式）
      if (response.project_id) {
        // 新格式：Project已创建，立即重置表单
        setCurrentTask(null)
        setDownloading(false)
        resetForm()
        ...
        message.success(`${platformName}Project创建Success，正在后台Download中，您可以继续添加其他Project`)
      ...
      setDownloading(false)
      const errorMessage = error.response?.data?.detail || error.message || '创建Download任务Failed'
  ...
  const stopDownload = () => {
    ...
    setDownloading(false)
    setCurrentTask(null)
    message.info('已Stop监控Download任务')
  }
          ...
          {videoInfo && (
            <div style={{
              ...
              <Text style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px' }}>
                {getVideoType(url) === 'bilibili' ? 'UP主' : '频道'}: {videoInfo.uploader || '未知'} • Duration: {videoInfo.duration ? `${Math.floor(videoInfo.duration / 60)}:${String(Math.floor(videoInfo.duration % 60)).padStart(2, '0')}` : '未知'}
              </Text>
            </div>
          )}
          
          {/* 只有解析Success后才显示ProjectName和分类 */}
          {videoInfo && (
            <>
              <div>
                <Text style={{ color: '#ffffff', marginBottom: '12px', display: 'block', fontSize: '16px', fontWeight: 500 }}>ProjectName（可选）</Text>
                <Input
                  placeholder="留空将使用视频Title作为ProjectName"
                  value={projectName}
              ...
              <div>
                <Text style={{ color: '#ffffff', marginBottom: '12px', display: 'block', fontSize: '16px', fontWeight: 500 }}>浏览器选择（获取AISubtitles需要）</Text>
                <Select
                ...
                <Text style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px', marginTop: '8px', display: 'block' }}>
                  选择浏览器可获取登录Status，用于DownloadAISubtitles。如不选择将只能Download公开Subtitles。
                </Text>
              </div>
